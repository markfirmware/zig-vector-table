...
fn link(comptime model: type) void {
    ...
    prepare_memory.line("", .{});
    externs.close();
    externs = IndentedStream.openRead(externs_file_name);
    _ = externs.readLine();
    while (externs.readLine()) |line| {
        prepare_memory.line("{}", .{line});
    }
...
pub fn linkSections(comptime options: SectionOptions, patterns: []const []const u8) void {
    if (options.prepare_by_copying_from) |mem| {
        ...
        externs.line("\nextern var __{}_start: u8;", .{options.name});
        externs.line("extern var __{}_end: u8;", .{options.name});
        externs.line("extern var __{}_load_start: u8;", .{options.name});
        ...
        script.line("__{}_start = .;", .{options.name});
        externs.line("", .{});
        externs.line("extern var __{}_start: u8;", .{options.name});
        externs.line("extern var __{}_end: u8;", .{options.name});
        prepare_memory.begin("{}:", .{options.name});
        prepare_memory.line("const start = &__{}_start;", .{options.name});
        prepare_memory.line("const end = &__{}_end;", .{options.name});
