...
fn link(builder: *std.build.Builder, comptime model: type) void {
    ...
    externs = std.ArrayList(u8).init(builder.allocator);
    ...
    prepare_memory.end("", .{});
    prepare_memory.line("{}", .{externs.items});
...
pub fn linkSections(comptime options: SectionOptions, patterns: []const []const u8) void {
    if (options.prepare_by_copying_from) |mem| {
        ...
        externs.outStream().print("\nextern var __{}_start: u8;\n", .{options.name}) catch unreachable;
        externs.outStream().print("extern var __{}_end: u8;\n", .{options.name}) catch unreachable;
        externs.outStream().print("extern var __{}_load_start: u8;\n", .{options.name}) catch unreachable;
        prepare_memory.begin("{}:", .{options.name});
        prepare_memory.line("const start = &__{}_start;", .{options.name});
        prepare_memory.line("const end = &__{}_end;", .{options.name});
        prepare_memory.line("const slice = @ptrCast([*]u8, start)[0 .. @ptrToInt(end) - @ptrToInt(start)];", .{});
        prepare_memory.line("const load_start = &__{}_load_start;", .{options.name});
...
const IndentedStream = struct {
    ...
    pub fn openRead(file_name: []const u8) IndentedStream {
        var self: IndentedStream = undefined;
        self.indent = 0;
        self.file = fs.cwd().openFile(file_name, fs.File.OpenFlags{}) catch unreachable;
        self.in = self.file.inStream();
        return self;
    }
    ...
    pub fn readLine(self: *IndentedStream) ?[]const u8 {
        return self.in.readUntilDelimiterOrEof(&self.in_buffer, 10) catch unreachable;
    }
    file: fs.File,
    in: std.io.InStream(fs.File, std.os.ReadError, fs.File.read),
    indent: u32,
    in_buffer: [200]u8,
...
var externs: std.ArrayList(u8) = undefined;
