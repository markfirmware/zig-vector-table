...
pub fn linkSections(comptime options: Section) void {
    if (options.prepare_by_copying_from) |mem| {
        script.begin(".determine_load_start_for_{s} :", .{options.name});
        script.line("__{s}_load_start = .;", .{options.name});
        script.end(" > {s}", .{mem.name});
        script.begin(".{s} : AT(__{s}_load_start)", .{ options.name, options.name });
        script.line("__{s}_start = .;", .{options.name});
        prepare_memory.begin("_ = {s}:", .{options.name});
        prepare_memory.begin("const e = struct", .{});
        prepare_memory.line("extern var __{s}_start: u8;", .{options.name});
        prepare_memory.line("extern var __{s}_end: u8;", .{options.name});
        prepare_memory.line("extern var __{s}_load_start: u8;", .{options.name});
        prepare_memory.end(";", .{});
        prepare_memory.line("const start = &e.__{s}_start;", .{options.name});
        prepare_memory.line("const end = &e.__{s}_end;", .{options.name});
        prepare_memory.line("const slice = @ptrCast([*]u8, start)[0 .. @ptrToInt(end) - @ptrToInt(start)];", .{});
        prepare_memory.line("const load_start = &e.__{s}_load_start;", .{options.name});
        ...
        prepare_memory.line("break :{s} 0;", .{options.name});
        prepare_memory.end(";", .{});
    } else if (options.prepare_by_setting_to_zero) {
        script.begin(".{s} (NOLOAD) :", .{options.name});
        script.line("__{s}_start = .;", .{options.name});
        prepare_memory.begin("_ = {s}:", .{options.name});
        prepare_memory.begin("const e = struct", .{});
        prepare_memory.line("extern var __{s}_start: u8;", .{options.name});
    ...
    if (options.prepare_by_setting_to_zero or options.prepare_by_copying_from != null) {
        script.line("__{s}_end = .;", .{options.name});
    }
