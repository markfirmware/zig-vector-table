...
pub fn prepareMemory() void {
    data_loaded_in_flash_that_program_must_copy_to_ram: {
        const start = &__data_loaded_in_flash_that_program_must_copy_to_ram_start;
        const end = &__data_loaded_in_flash_that_program_must_copy_to_ram_end;
        const slice = @ptrCast([*]u8, start)[0 .. @ptrToInt(end) - @ptrToInt(start)];
        const load_start = &__data_loaded_in_flash_that_program_must_copy_to_ram_load_start;
        const loaded_slice = @ptrCast([*]u8, load_start)[0..slice.len];
        std.mem.copy(u8, slice, loaded_slice);
    }
    data_that_program_must_set_to_zero: {
        const start = &__data_that_program_must_set_to_zero_start;
        const end = &__data_that_program_must_set_to_zero_end;
        const slice = @ptrCast([*]u8, start)[0 .. @ptrToInt(end) - @ptrToInt(start)];
